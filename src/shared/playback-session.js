(function attachPlaybackSessionModule(root, factory) {
  const api = factory();

  if (typeof module === "object" && module.exports) {
    module.exports = api;
  } else {
    root.YTResumePlaybackSession = Object.freeze(api);
  }
})(typeof globalThis !== "undefined" ? globalThis : this, function createPlaybackSessionModule() {
  "use strict";

  function createPlaybackSessionController({
    environment = globalThis,
    logic = environment.YTResume,
    browserApi = environment.YTResumeBrowser,
  } = {}) {
    const {
      browser,
      crypto,
    ...
    const { addMessageListener, sendRuntimeMessage } = browserApi;
    ...
    const PLAYER_DISCOVERY_TIMEOUT_MS = 30000;
    const PLAYER_READY_TIMEOUT_MS = 120000;
      ...
      return new Promise((resolve) => {
        function check() {
          ...
          const playerContextMatches = (pageContextMatches && durationMatches)
            || elapsed >= PLAYER_CONTEXT_SETTLE_TIMEOUT_MS;
          ...
          const playerWaitTimedOut = elapsed >= (video
            ? PLAYER_READY_TIMEOUT_MS
            : PLAYER_DISCOVERY_TIMEOUT_MS);
          if (playerWaitTimedOut) {
            resolve(null);
            return;
          }
    ...
    async function persistSessionPayload(session, payload, { persistAfterStop = false } = {}) {
      if (!persistAfterStop && !isCurrentSessionForLocation(session)) {
        return null;
      }
    ...
    async function saveSession(session, { force = false, persistAfterStop = false } = {}) {
      const canSave = isCurrentSessionForLocation(session)
        || (persistAfterStop && currentSession === session && !session.destroyed);
      if (
        !canSave
        || !session.video
      ...
      if (!session.saveQueue) {
        session.saveQueue = createLatestTaskQueue((task) => persistSessionPayload(
          session,
          task.payload,
          { persistAfterStop: task.persistAfterStop },
        ));
      }
      const result = await session.saveQueue.enqueue({ payload, persistAfterStop });
    ...
    function stopCurrentSession(save = true) {
      ...
      if (save) {
        void saveSession(session, { force: true, persistAfterStop: true });
      }
    ...
    async function startForCurrentUrl(force = false) {
      ...
      let record;
      try {
        const initialState = await sendMessage("session:get", { videoId: context.videoId });
        settings = normalizeSettings(initialState?.settings);
        record = initialState?.record || null;
      } catch {
      ...
      if (!video) {
        if (isCurrentSessionForLocation(session)) {
          publicState.reason = "Video player unavailable. Reload YouTube to try again.";
        }
        return;
      }
      if (!isCurrentSessionForLocation(session)) {
      ...
      if (record && shouldRestoreCheckpoint(context, record.position)) {
    ...
    function handlePageHide() {
      if (currentSession) {
        void saveSession(currentSession, { force: true, persistAfterStop: true });
      }
    }
    ...
    function start() {
      ...
      isStarted = true;
      removeRuntimeMessageListener = addMessageListener(handleRuntimeMessage);
      browser.storage.onChanged.addListener(handleStorageChange);

      navigationObserver = new MutationObserver(checkForLocationChange);
    ...
    function dispose() {
      ...
      if (typeof removeRuntimeMessageListener === "function") {
      ...
      window.removeEventListener("hashchange", checkForLocationChange);
      window.removeEventListener("popstate", checkForLocationChange);
      window.removeEventListener("pageshow", checkForLocationChange);
      window.removeEventListener("pagehide", handlePageHide);
      stopCurrentSession(true);
      removeToast(true);
    }

    return Object.freeze({ dispose, start });
  }

  return Object.freeze({ createPlaybackSessionController });
});
